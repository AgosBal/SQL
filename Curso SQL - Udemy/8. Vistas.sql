...
as select concat(apellido, ' ', e.nombre) as nombre, sexo, s.nombre as seccion, cantidadhijos
from empleados as e 
join secciones as s 
on codigo=seccion;
...
as select extract(year from fechaingreso) as fecingreso, count(*) as cantidad
from empleados
group by fecingreso;
...
create view vista_empleado_con_hijos
as select nombre, sexo, seccion, cantidadhijos
from vista_empleado
...
select * from vista_nota_alumnos_aprodabos;
 ...
 update vista_nota_alumnos_aprodabos
 set nota=10
